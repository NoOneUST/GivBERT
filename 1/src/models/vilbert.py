...
class BertPreTrainingHeads(nn.Module):
    ...
    def forward(
        self, sequence_output_t, sequence_output_v, pooled_output_t, pooled_output_v, raw_image
    ):
        ...
        prediction_scores_v = self.imagePredictions(sequence_output_v, raw_image)
...
class BertImagePredictionHead(nn.Module):
    def __init__(self, config):
        ...
        self.global_extract = VisionGlobalFeatureExtract(output_dim=config.v_hidden_size)

    def forward(self, hidden_states, raw_image):
        hidden_states = self.transform(hidden_states)
        hidden_states = self.decoder(hidden_states + self.global_extract(raw_image).unsqueeze(1).repeat(1,hidden_states.shape[1],1))
        return hidden_states

    # def forward(self, hidden_states, raw_image):
    #     hidden_states = self.transform(hidden_states)
    #     hidden_states = self.decoder(hidden_states + self.global_extract(raw_image))
    #     return hidden_states
...
class VILBertForVLTasks(BertPreTrainedModel):
    def __init__(self, config, num_labels, dropout_prob=0.1, default_gpu=True):
        ...
        # self.vision_feature_extract = VisionGlobalFeatureExtract(output_dim=config.v_hidden_size)
    ...
    def forward(
        ...
        linguisic_prediction, vision_prediction = self.cls(
            sequence_output_t, sequence_output_v, pooled_output_t, pooled_output_v, raw_image=raw_image
        )
...
class VisionGlobalFeatureExtract(nn.Module):
    def __init__(self, output_dim):
        super().__init__()
        self.vision_global_model = torch.hub.load('pytorch/vision:v0.6.0', 'mobilenet_v2', pretrained=True)
        self.vision_global_model.classifier = nn.Sequential(*[nn.Linear(in_features=1280, out_features=output_dim, bias=True), nn.ReLU(), nn.Linear(in_features=output_dim, out_features=output_dim, bias=True), nn.ReLU()])
        ...
        for param in self.vision_global_model.features:
            param.requires_grad = False
        
    def forward(self, input):
        return self.vision_global_model(input)
